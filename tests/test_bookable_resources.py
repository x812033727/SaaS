...
def test_owner_manages_resources_in_backoffice_and_staff_is_read_only(client, db):
    tenant_id, email = _login_owner(client)
    page = client.get("/ui/resources")
    assert page.status_code == 200
    assert "如何設定自動配置" in page.text

    response = client.post(
        "/ui/resources/types",
        data={"name": "獨立包廂", "description": "隱私療程使用"},
    )
    assert response.status_code == 200
    assert "獨立包廂" in response.text
    with _Session() as session:
        resource_type = resources_svc.list_types(
            session, tenant_id=tenant_id
        )[0]
        service = session.query(Service).filter_by(tenant_id=tenant_id).one()

    response = client.post(
        "/ui/resources",
        data={
            "resource_type_id": str(resource_type.id),
            "name": "包廂 A",
            "internal_code": "ROOM-A",
            "capacity": "1",
            "location_id": "",
            "available_from": "",
            "available_until": "",
    ...
    assert response.status_code == 200
    assert "包廂 A" in response.text
    response = client.post(
        "/ui/resource-requirements",
        data={
    ...
    assert response.status_code == 200
    assert "頭皮護理" in response.text

    with _Session() as session:
        user = session.query(User).filter_by(email=email).one()
        user.role = "staff"
        session.commit()
    assert client.get("/ui/resources").status_code == 200
    assert client.post(
        "/ui/resources/types", data={"name": "不應建立", "description": ""}
    ).status_code == 403
