...
class TestSlotDurationForm:
    def test_ui_create_slot_with_duration(self, app_client):
        import uuid as _uuid

        client, _line = app_client
        email = f"slot_{_uuid.uuid4().hex[:8]}@example.com"
        r = client.post("/auth/register", json={
            "email": email, "password": "Test1234!",
            "tenant_name": f"slot_t_{_uuid.uuid4().hex[:8]}",
        })
        assert r.status_code == 201
        client.post("/ui/login", data={"email": email, "password": "Test1234!"},
                    follow_redirects=False)
        r2 = client.post("/ui/booking/slots", data={
            "slot_start": "2030-06-01T10:00",
            "duration_minutes": "45",
            "max_capacity": "4",
            "walkin_reserved": "0",
        })
        ...
        try:
            slot = db.execute(
                select(BookingSlot).order_by(BookingSlot.id.desc())
            ).scalars().first()
            assert slot.slot_end is not None
            assert (slot.slot_end - slot.slot_start) == datetime.timedelta(minutes=45)
        finally:
            db.close()

    def test_ui_create_slot_invalid_duration_shows_error(self, app_client):
        import uuid as _uuid

        client, _line = app_client
        email = f"slot2_{_uuid.uuid4().hex[:8]}@example.com"
        client.post("/auth/register", json={
            "email": email, "password": "Test1234!",
            "tenant_name": f"slot2_t_{_uuid.uuid4().hex[:8]}",
        })
        client.post("/ui/login", data={"email": email, "password": "Test1234!"},
                    follow_redirects=False)
