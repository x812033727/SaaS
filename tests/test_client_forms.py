...
def test_owner_can_build_and_activate_form_but_staff_cannot_mutate(client, db):
    tenant_id, email = _login_owner(client)
    with _Session() as session:
        service = Service(
            tenant_id=tenant_id,
            name="後台頭皮護理",
        ...
        customer = Customer(
            tenant_id=tenant_id,
            line_user_id=f"U-existing-{uuid.uuid4().hex[:8]}",
            display_name="既有預約顧客",
        )
        slot = BookingSlot(
            tenant_id=tenant_id,
            slot_start=datetime.datetime(
                2033, 6, 1, 10, tzinfo=datetime.timezone.utc
            ),
            max_capacity=1,
        )
        session.add_all([service, customer, slot])
        session.commit()
        service_id, customer_id, slot_id = service.id, customer.id, slot.id
        line_user_id = customer.line_user_id
    ...
    page = client.get("/ui/client-forms")
    assert page.status_code == 200
    assert "建立表單範本" in page.text
    ...
    created = client.post(
        ...
        data={
            "name": "後台諮詢同意書",
            "intro": "請先填寫",
            "consent_text": "本人確認資料真實並同意接受本次服務與注意事項。",
            "service_id": str(service_id),
            "require_signature": "true",
        },
    )
    assert created.status_code == 200
    assert "後台諮詢同意書" in created.text
    assert "後台頭皮護理" in created.text

    with _Session() as session:
        template = forms_svc.list_templates(session, tenant_id=tenant_id)[0]
        template_id = template.id

    question = client.post(
        f"/ui/client-forms/{template_id}/questions",
        data={
            "label": "是否有過敏史",
            "field_type": "select",
            "required": "true",
    ...
    assert "是否有過敏史" in question.text
    activated = client.post(
        f"/ui/client-forms/{template_id}/active", data={"active": "true"}
    )
    assert activated.status_code == 200
    assert "派發中" in activated.text

    with _Session() as session:
        assert session.query(ClientFormRequest).filter_by(
            reservation_id=reservation_id
        ).count() == 0
    attached = client.post(
        f"/ui/customers/{customer_id}/reservations/{reservation_id}/client-forms"
    )
    assert attached.status_code == 200
    assert f"預約 #{reservation_id}" in attached.text
    with _Session() as session:
        assert session.query(ClientFormRequest).filter_by(
            reservation_id=reservation_id
        ).count() == 1
    # 補派採預約+範本唯一鍵，重複點擊仍只有一份。
    assert client.post(
        f"/ui/customers/{customer_id}/reservations/{reservation_id}/client-forms"
    ).status_code == 200
    with _Session() as session:
    ...
    with _Session() as session:
        ...
        assert {
            "client_forms.create",
            "client_forms.question.add",
            "client_forms.active",
            "client_forms.attach",
        }.issubset(actions)
        session.query(User).filter_by(email=email).one().role = "staff"
        session.commit()

    forbidden = client.post(
        f"/ui/client-forms/{template_id}/questions",
        data={"label": "越權問題", "field_type": "text"},
    )
