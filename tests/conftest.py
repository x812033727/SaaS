...
# ── 測試提速：bcrypt 預設 12 rounds，每次 hash/verify ~0.3s，數百個 register/login
# 測試累積使全套逼近 60s self-test 逾時。測試環境把 rounds 降到最低值 4，
# 雜湊行為與格式不變（仍是 bcrypt），僅迭代次數變少 → 全套大幅加速。
# 這只在 test session 生效（conftest 只在測試載入），不影響 production 設定。
