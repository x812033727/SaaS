...
| AI 客服 | `AI_ASSISTANT` | `/ai` | FAQ 比對 + Claude Agent SDK／MiniMax 問答 |
...
| `SAAS_MINIMAX_BASE_URL` | MiniMax 國際站 Anthropic-compatible endpoint | `https://api.minimax.io/anthropic` |
| `SAAS_AI_MODEL` | 後台未設定時使用的 MiniMax 模型備援值 | `MiniMax-M2.7` |
...
使用 `claude-agent-sdk` 的 bundled Claude Code CLI，並依 MiniMax 官方 Claude Code
方式連到 `https://api.minimax.io/anthropic`。
