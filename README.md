...
| AI 客服 | `AI_ASSISTANT` | `/ai` | FAQ 比對 + MiniMax Direct API 問答 |
...
| 定金收款 | `DEPOSIT_PAYMENT`（pro） | `/payments/deposit/{trade_no}`（stub/綠界/藍新/LINE Pay 依設定分派） | 防 no-show：線上建單收定金，逾時自動取消回補名額 |
...
| `/ui/admin/observability-settings` | 平台管理員：加密設定 Sentry DSN、送出測試事件並立即啟用錯誤監控 |
...
| `SAAS_MINIMAX_API_KEY` | MiniMax 金鑰備援值；後台「平台管理 → AI 設定」的加密資料庫設定優先 | `""` |
| `SAAS_MINIMAX_BASE_URL` | MiniMax 國際站 OpenAI-compatible endpoint | `https://api.minimax.io/v1` |
| `SAAS_AI_MODEL` | 後台未設定時使用的 MiniMax 模型備援值 | `MiniMax-M3` |
| `SAAS_SENTRY_DSN` | Sentry DSN 備援值；後台「平台管理 → 錯誤監控設定」的加密資料庫設定優先 | `""` |
...
平台管理員可到 `/ui/admin/ai-settings` 設定 MiniMax API Key 與模型；正式 AI 路徑
使用官方支援的 OpenAI Python SDK 直接呼叫 MiniMax API，連到
`https://api.minimax.io/v1`，不經過 Claude Code 或 Anthropic 服務。
API Key 以 Fernet 加密保存，頁面與稽核紀錄不顯示明文；儲存後 AI 客服、LINE
自由文字回覆與 AI 預約助理立即改用 MiniMax。頁面提供極小用量的連線測試，能分辨
金鑰、模型、額度或連線問題。移除資料庫設定後會使用 `SAAS_MINIMAX_*` 備援；若
備援也未設定，安全退回 FAQ 規則模式，既有 FAQ 不受影響。

### 平台錯誤監控設定

平台管理員可到 `/ui/admin/observability-settings` 加密保存 Sentry DSN，儲存後立即
重載錯誤監控，並可送出不含憑證的測試事件。移除資料庫設定後會使用
`SAAS_SENTRY_DSN` 備援；兩者皆未設定時，告警仍會保留在應用程式 error log。
